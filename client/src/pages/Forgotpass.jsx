...
export default function Forgotpass() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const request = await post("/api/auth/updatePassword", { email, password });
  ...
  <div className="w-full max-w-md p-8 bg-gray-900 rounded-lg shadow-md animate__animated animate__bounceIn">
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="animate__animated animate__fadeInUp">
        <label htmlFor="email" className="block text-sm font-medium text-gray-300 mb-1">Create New Password</label>
        <input
          type="email"
          id="email"
      ...
      <div className="animate__animated animate__fadeInUp">
        <label htmlFor="password" className="block text-sm font-medium text-gray-300 mb-1">Confirm New Password</label>
        <input
          type="password"
          id="password"
          className="w-full px-4 py-2 bg-gray-800 text-gray-300  rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
